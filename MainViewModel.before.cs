...
using EvenniaMapMaker.Services;
using Microsoft.Win32;

namespace EvenniaMapMaker.ViewModels;
...
public class MainViewModel : BaseViewModel
{
    ...
    public int CurrentZ { get => _currentZ; set { SetField(ref _currentZ, value); OnPropertyChanged(nameof(FloorLabel)); } }
    ...
    public string TitleBarText { get { var n = string.IsNullOrEmpty(_project.Name) ? "Untitled" : _project.Name; return $"Evennia Map Maker - {n}{(_isDirty ? " *" : "")}"; } }
    public string StatusText { get { var sel = SelectedRoom; if (sel != null) return $"Room: {sel.Id}  ({sel.X}, {sel.Y}, {sel.Z})"; return $"Rooms: {Rooms.Count}  Conns: {Connections.Count}  Floor: {_currentZ}"; } }
    ...
    public ObservableCollection<ConnectionModel> RoomConnections { get { var list = new ObservableCollection<ConnectionModel>(); if (_selectedRoom != null) foreach (var c in Connections.Where(c => c.SourceRoomId == _selectedRoom.Id || c.DestinationRoomId == _selectedRoom.Id)) list.Add(c); return list; } }
    ...
    private void SyncDoorWithReverse(ConnectionModel conn) { if (string.IsNullOrEmpty(conn.SharedDoorId)) return; var rev = Connections.FirstOrDefault(c => c.SharedDoorId == conn.SharedDoorId && c.Id != conn.Id); if (rev != null && conn.Door != null) { rev.ExitType = conn.ExitType; rev.Door = new DoorModel { Name = conn.Door.Name, StartsOpen = conn.Door.StartsOpen, StartsClosed = conn.Door.StartsClosed, StartsLocked = conn.Door.StartsLocked, KeyId = conn.Door.KeyId }; } }

    private void FireRoomPropsChanged() { OnPropertyChanged(nameof(IsRoomSelected)); OnPropertyChanged(nameof(IsConnectionSelected)); OnPropertyChanged(nameof(SelectedRoomTitle)); OnPropertyChanged(nameof(SelectedRoomType)); OnPropertyChanged(nameof(SelectedRoomDescription)); OnPropertyChanged(nameof(SelectedRoomTags)); OnPropertyChanged(nameof(SelectedRoomNotes)); OnPropertyChanged(nameof(SelectedRoomX)); OnPropertyChanged(nameof(SelectedRoomY)); OnPropertyChanged(nameof(SelectedRoomZ)); OnPropertyChanged(nameof(RoomConnections)); }

    private void FireConnPropsChanged() { OnPropertyChanged(nameof(SelectedConnectionDirection)); OnPropertyChanged(nameof(SelectedConnectionAliases)); OnPropertyChanged(nameof(SelectedConnectionExitType)); OnPropertyChanged(nameof(SelectedConnectionIsOneWay)); OnPropertyChanged(nameof(SelectedConnectionDoorName)); OnPropertyChanged(nameof(SelectedConnectionStartsOpen)); OnPropertyChanged(nameof(SelectedConnectionStartsClosed)); OnPropertyChanged(nameof(SelectedConnectionStartsLocked)); OnPropertyChanged(nameof(SelectedConnectionKeyId)); OnPropertyChanged(nameof(IsDoorSelected)); OnPropertyChanged(nameof(ShowDoorProperties)); }

    private void RefreshStatus() { OnPropertyChanged(nameof(StatusText)); }

    public RoomModel? CreateRoomAt(int x, int y, int z) { if (Rooms.Any(r => r.X == x && r.Y == y && r.Z == z)) return null; _roomCounter++; var room = new RoomModel { Id = _project.Id + "_room_" + _roomCounter.ToString("D4"), X = x, Y = y, Z = z }; Rooms.Add(room); _project.Rooms = Rooms.ToList(); MarkDirty(); PushUndo("Add " + room.Id, () => { Rooms.Remove(room); _project.Rooms = Rooms.ToList(); MapNeedsRefresh?.Invoke(); }); MapNeedsRefresh?.Invoke(); SelectedRoom = room; RefreshStatus(); return room; }


    public void NavigateOrBuild(Direction dir) { var sel = SelectedRoom; if (sel == null) return; var (nx, ny, nz) = DirectionHelper.GetNeighborCoordinate(sel, dir); var existing = Rooms.FirstOrDefault(r => r.X == nx && r.Y == ny && r.Z == nz); if (existing != null) { if (!Connections.Any(c => c.SourceRoomId == sel.Id && c.DestinationRoomId == existing.Id)) CreateConnection(sel, existing, dir); SelectedRoom = existing; if (existing.Z != CurrentZ) CurrentZ = existing.Z; MapNeedsRefresh?.Invoke(); } else if (_buildMode) { var r = CreateRoomAt(nx, ny, nz); if (r != null) { CreateConnection(sel, r, dir); if (r.Z != CurrentZ) CurrentZ = r.Z; } } }
    public void CreateConnection(RoomModel src, RoomModel dst, Direction direction, ExitType exitType = ExitType.Normal) { if (Connections.Any(c => c.SourceRoomId == src.Id && c.DestinationRoomId == dst.Id && c.Direction == direction)) return; _connectionCounter++; var doorId = exitType == ExitType.Door ? "door_" + _connectionCounter.ToString("D4") : ""; var conn = new ConnectionModel { Id = "connection_" + _connectionCounter.ToString("D4"), SourceRoomId = src.Id, DestinationRoomId = dst.Id, Direction = direction, ReverseDirection = DirectionHelper.GetReverseDirection(direction), ExitType = exitType, SharedDoorId = doorId, Door = exitType == ExitType.Door ? new DoorModel { StartsClosed = true } : null, AutoCreateReverse = _autoReverse }; Connections.Add(conn); _project.Connections = Connections.ToList(); if (_autoReverse && !conn.IsOneWay) { _connectionCounter++; Connections.Add(new ConnectionModel { Id = "connection_" + _connectionCounter.ToString("D4"), SourceRoomId = dst.Id, DestinationRoomId = src.Id, Direction = DirectionHelper.GetReverseDirection(direction), ReverseDirection = direction, ExitType = exitType, SharedDoorId = doorId, Door = exitType == ExitType.Door ? new DoorModel { Name = conn.Door?.Name ?? "", StartsOpen = conn.Door?.StartsOpen ?? false, StartsClosed = conn.Door?.StartsClosed ?? true, StartsLocked = conn.Door?.StartsLocked ?? false, KeyId = conn.Door?.KeyId ?? "" } : null }); _project.Connections = Connections.ToList(); } PushUndo("Connect " + conn.Id, () => { Connections.Remove(conn); var rev = Connections.FirstOrDefault(c => c.SharedDoorId == doorId && c.Id != conn.Id); if (rev != null) Connections.Remove(rev); _project.Connections = Connections.ToList(); MapNeedsRefresh?.Invoke(); }); MarkDirty(); MapNeedsRefresh?.Invoke(); }
    ...
    public void DeleteRoom(RoomModel room) { var cons = Connections.Where(c => c.SourceRoomId == room.Id || c.DestinationRoomId == room.Id).ToList(); foreach (var c in cons) Connections.Remove(c); Rooms.Remove(room); _project.Rooms = Rooms.ToList(); _project.Connections = Connections.ToList(); PushUndo("Delete " + room.Id, () => { Rooms.Add(room); foreach (var c in cons) Connections.Add(c); _project.Rooms = Rooms.ToList(); _project.Connections = Connections.ToList(); MapNeedsRefresh?.Invoke(); }); SelectedRoom = null; MarkDirty(); MapNeedsRefresh?.Invoke(); }

    private void DeleteConnection(ConnectionModel conn) { var rev = Connections.FirstOrDefault(c => c.SharedDoorId == conn.SharedDoorId && c.Id != conn.Id); Connections.Remove(conn); if (rev != null) Connections.Remove(rev); _project.Connections = Connections.ToList(); PushUndo("Delete " + conn.Id, () => { Connections.Add(conn); if (rev != null) Connections.Add(rev); _project.Connections = Connections.ToList(); MapNeedsRefresh?.Invoke(); }); SelectedConnection = null; MarkDirty(); MapNeedsRefresh?.Invoke(); }
    ...
    public void StartNewProject() { _project = new MapProject { Id = "untitled", Name = "Untitled", Version = 1 }; _currentFilePath = null; _isDirty = false; _roomCounter = 0; _connectionCounter = 0; Rooms.Clear(); Connections.Clear(); _undoStack.Clear(); _redoStack.Clear(); SelectedRoom = null; SelectedConnection = null; CurrentZ = 0; NotifyProjectChanged(); MapNeedsRefresh?.Invoke(); }

    public void NewProject() { if (!ConfirmDiscardChanges()) return; var dlg = new EvenniaMapMaker.Dialogs.NewProjectDialog(); if (dlg.ShowDialog() == true) { _project = new MapProject { Id = dlg.AreaId, Name = dlg.AreaName, Version = 1 }; _currentFilePath = null; _isDirty = false; _roomCounter = 0; _connectionCounter = 0; Rooms.Clear(); Connections.Clear(); _undoStack.Clear(); _redoStack.Clear(); SelectedRoom = null; SelectedConnection = null; CurrentZ = 0; NotifyProjectChanged(); MapNeedsRefresh?.Invoke(); } }

    public void OpenProject() { if (!ConfirmDiscardChanges()) return; var dlg = new OpenFileDialog { Filter = "Evennia Map Files (*.evenniamap)|*.evenniamap", Title = "Open Map Project" }; if (dlg.ShowDialog() == true) { try { _project = _fileService.Load(dlg.FileName); _currentFilePath = dlg.FileName; _isDirty = false; Rooms.Clear(); Connections.Clear(); foreach (var r in _project.Rooms) Rooms.Add(r); foreach (var c in _project.Connections) Connections.Add(c); _roomCounter = Rooms.Count > 0 ? Rooms.Max(r => int.TryParse(r.Id.Split(Convert.ToChar(95)).Last(), out var n) ? n : 0) : 0; _connectionCounter = Connections.Count > 0 ? Connections.Max(c => int.TryParse(c.Id.Split(Convert.ToChar(95)).Last(), out var n) ? n : 0) : 0; _undoStack.Clear(); _redoStack.Clear(); SelectedRoom = null; SelectedConnection = null; CurrentZ = 0; NotifyProjectChanged(); MapNeedsRefresh?.Invoke(); } catch (Exception ex) { MessageBox.Show("Failed to open: " + ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error); } } }
